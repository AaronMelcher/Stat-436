...
library(scico)
ggplot(centroids) +
  geom_tile(aes(reorder(question, value, var), reorder(cluster, value), fill=value, col=value)) +
  scale_fill_scico("bam0", midpoint = 0.5) +
  scale_color_scico("bam0", midpoint = 0.5)
```
...
slope_fun <- function(x) {
 ...
 bind_cols(cluster = fit$cluster, survery_wider) |>
